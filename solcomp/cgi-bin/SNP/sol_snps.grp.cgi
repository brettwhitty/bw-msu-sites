...
## fetch the coordinates for each of the members of the alignment
my @member_coords = $aln_util->get_member_coords($aln);

## create the overview image
my ($snp_loc_png, $img_map_html) = generate_snp_location_png($snp_asm, \@member_coords, \@snps);
...
if ($grp) {
    ...
    $tmpl->param("has_snps"     => (scalar(@{$snp_asm->{'snps'}}) > 0) ? 1 : 0);
...
#    $tmpl->param("function"     => (defined($put_function{$snp_asm->{'put_id'}}))
#                                   ? $put_function{$snp_asm->{'put_id'}} : 'N/A');
...
sub generate_snp_location_png {
    my ($snp_asm, $members_arr_ref, $snps_arr_ref) = @_;
    ...
    ## add member ESTs to image
    foreach my $member(@{$members_arr_ref}) {
        $panel->add_track(
                    arrow => Bio::SeqFeature::Generic->new(
...
sub create_link {
    ...
    if (! defined($feature->display_name) || $feature->display_name =~ /^PUT/i || $feature->display_name =~ /^\d+$/) {
        return undef;
    } else {
        return '#';
    }
