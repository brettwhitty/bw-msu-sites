    ...
    if ( this.sort_column >= 0 &&
      ...
      if (header.retrieve('conversion_function')) {
        ...
        rows.some(function(row){
          var to_match = $(row.row.getElementsByTagName('td')[this.sort_column]).get('text').trim();
    ...
    this.conversions = $A([
      ...
      { matcher: /([0-9.]{1,8}).*([KMGT]{1})B/,
        conversion_function: function( row ) {
          var cell = $(row.row.getElementsByTagName('td')[this.sort_column]).get('text').trim();
      ...
      { matcher: /.*/,
        conversion_function: function( row ) {
          return $(row.row.getElementsByTagName('td')[this.sort_column]).get('text').trim();
        }
      }
    ]);
  }
